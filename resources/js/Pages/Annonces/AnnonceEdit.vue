    ...
    <div>
        ...
        <div class="annonce-header">
            <div class="annonce-header-text">
                ...
                <button class="button-post background-blue">
                    POSTULER
                </button>
                <button v-if="user.id == annonce.compagnies_id" class="button-post background-red" @click="Delete">
                    Supprimer L'annonce
                </button>
            </div>
        ...
        <div class="annonce-content">
            <div class="h-11 mt-8 mb-8">
                <h3>Service :</h3>
                <input id="announcement_title" type="text" placeholder="Service" class="block w-full" required
                    :value="annonce.announcement_title" />
            </div>

            <div class="h-11 mt-8 mb-8">
                <h3>Type de contrat :</h3>
                <input id="type_of_contract" type="text" placeholder="CDD/CDI" class="block w-full" required
                    :value="annonce.type_of_contract" />
            </div>

            <div class="h-11 mt-8 mb-8">
                <h3>Où se situe votre entreprise:</h3>
                <input id="localisation" type="text" placeholder="localisation" class="block w-full" required
                    :value="annonce.localisation" />
            </div>

            <div class="h-11 mt-8 mb-8">
                <h3>Domaine d'activité :</h3>
                <input id="domaine" type="text" placeholder="Dans quelle domaine vous cherchez" class="block w-full"
                    required :value="annonce.domaine" />
            </div>

            <div class="h-11 mt-8 mb-8">
                <h3>Temps de travail :</h3>
                <input id="time_work" type="number" placeholder="Combien d'heure par semaine" class="block w-full"
                    required :value="annonce.time_work" />
            </div>

            <div class="h-11 mt-8 mb-8">
                <h3>Nombre de post ouvert :</h3>
                <input id="number_of_jobs" type="number" placeholder="nombre de post resant" class="block w-full"
                    required :value="annonce.number_of_jobs" />
            </div>

            <div class="h-11 mt-8 mb-8">
                <h3>Rémuneration :</h3>
                <input id="pay" type="number" placeholder="Salaire/mois" class="block w-full" required
                    :value="annonce.pay" />
            </div>
    ...
    export default {
        ...
        methods: {
            submit() {
                ...
                this.$inertia.put(this.route('annonce_compagnie.update', this.annonce.id), this.form)
        }
    }
